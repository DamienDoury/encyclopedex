    ...
    <head>
		...
		function actualiser_pokedex()
		{
			...
			$.ajax({ 
				...
				{
					if(data != "" && !data.match(/#Erreur/))
						window.history.replaceState("", "Pokédex - Encyclopedex", "/pokedex_tile/" + data);
					else
						window.history.replaceState("", "Pokédex - Encyclopedex", "/pokedex_tile/");
				}
			});
					...
					success: function(data)
					{
						if(numero_requete == nombre_requetes)
						{
							...
							for(var pkmn_nb = 0; pkmn_nb < j.length; pkmn_nb++) // On parcourt la liste des Pokemon.
							{
								...
								temp += "<div data-id='" + pkmn.id + "' class='resultat'" + (pkmn.form ? " data-forme='" + pkmn.form + "'" : "") + " style=''><div style='position: absolute; width: 96px; height: 96px; overflow: hidden; background: url(\"/images/tiles.png\") no-repeat " + img_pos + ";'></div>";
								
								if(pkmn.stats) // Si le l'objet pokemon contient des informations autres que l'id et la forme, on les affiche.
								{
									temp += "<div class='stats'><a>&nbsp;</a>";
